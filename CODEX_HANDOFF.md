...
- `LexWriterUITests/LexWriterUITests.swift`: UI test expectations updated; target application now points to `LexWriter`, but UI tests have not been run in this batch.
...
- UI test target now points to `LexWriter`; UI tests have not been run in this batch.
...
`LexWriterUITests` now has `LexWriter` as its target application. UI tests have not been run in this batch yet, so treat the first UI-test run as a separate validation step.

## Recommended Next Steps

1. Run UI tests as a separate validation step if UI automation should become part of the release gate.
