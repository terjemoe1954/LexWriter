...
- Current validation baseline: app build succeeds, build-for-testing succeeds, 83 unit tests pass, 7 UI tests pass, the launch test passes, and manual simulator smoke passes for Home, Settings, Premium preview, language switching, one free document, one premium-marked document, and empty Will preview validation. The latest separate validation run on 2026-09-22 after printable HTML signature page-break protection passed unit tests 83/83, build-for-testing, app build, UI tests 7/7, and launch test 1/1. `LexWriterUITests` now points to `LexWriter`.
