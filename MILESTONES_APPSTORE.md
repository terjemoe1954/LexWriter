...
- Current validation baseline: app build succeeds, build-for-testing succeeds, 84 unit tests pass, 7 UI tests pass, the launch test passes, and manual simulator smoke passes for Home, Settings, Premium preview, language switching, one free document, one premium-marked document, and empty Will preview validation. The latest separate validation run on 2026-09-22 after premium activation guide baseline cleanup passed unit tests 84/84, build-for-testing, app build, UI tests 7/7, and launch test 1/1. The latest unit-only validation run on 2026-09-22 after milestone coverage status sync passed unit tests 84/84, the latest build-for-testing validation after that succeeded, the latest app build validation after that succeeded, the latest UI-test validation after that passed 7/7, and the latest launch-test validation after that passed 1/1. `LexWriterUITests` now points to `LexWriter`.
