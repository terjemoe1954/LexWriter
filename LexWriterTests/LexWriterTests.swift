...
struct LexWriterTests {
    ...
    @Test func versionOneOnePlanKeepsPremiumActivationDeferred() async throws {
        ...
        #expect(plan.contains("automated gates are current as separate checks"))
        #expect(plan.contains("source-level print/pdf structure checks all passed on 2026-09-22"))
        #expect(plan.contains("manual app store connect or xcode organizer checks"))
        #expect(plan.contains("broader device smoke testing, visual exported-pdf review, and screenshot/metadata review"))
        ...
        #expect(plan.contains("run the launch test before archive"))
    ...
    @Test func codexHandoffKeepsSafeSessionInstructionsCurrent() async throws {
        ...
        #expect(handoff.contains("latest manual simulator smoke on 2026-09-22"))
        #expect(handoff.contains("premium preview unavailable-purchases copy"))
        #expect(handoff.contains("absent purchase/restore controls passed"))
        #expect(handoff.contains("latest source-level print/pdf review on 2026-09-22"))
        #expect(handoff.contains("visual exported-pdf inspection is still a separate manual checklist item"))
        ...
        #expect(handoff.contains("continue with manual 1.1 checks"))
        #expect(handoff.contains("broader device smoke testing, visual exported-pdf review, and screenshot/metadata review"))
    ...
    @Test func premiumActivationGuideKeepsActivationPreconditionsExplicit() async throws {
        ...
        #expect(guide.contains("lexwriteruitests` still points to `lexwriter`"))
